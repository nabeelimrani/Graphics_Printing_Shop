...
@section('main-section')
    @push('title')
        <title>Product View - Hassan Graphics & Printing</title>
    @endpush
                                ...
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tbody>
                                        @foreach ($product as $productdata)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <button class="btn btn-primary btn-sm" data-toggle="modal"
                                                        data-target="#editModal{{ $productdata->id }}">Edit</button>
                                                    <button class="btn btn-danger btn-sm" data-toggle="modal"
                                                        data-target="#deleteModal{{ $productdata->id }}">Delete</button>
                                                </td>
                                            </tr>
                                            <!-- Delete Modal -->
                                            <div class="modal fade" id="deleteModal{{ $productdata->id }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <form
                                                                action="{{ route('productdel', ['id' => $productdata->id]) }}"
                                                                method="POST">
                                                                @csrf
                                                                <button type="submit"
                                            ...
                                            <!-- Edit customer Modal -->
                                            <div class="modal fade" id="editModal{{ $productdata->id }}" tabindex="-1"
                                                role="dialog" aria-labelledby="editcustomerModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="editcustomerModalLabel">Edit
                                                                Product</h5>
                                                            <button type="button" class="close" data-dismiss="modal"
                                                                aria-label="Close">
                                                                <span aria-hidden="true">&times;</span>
                                                        ...
                                                        <div class="modal-body">
                                                            <form
                                                                action="{{ route('productedit', ['id' => $productdata->id]) }}"
                                                                method="POST">
                                                                @csrf
                                                                    ...
                                                                    <div class="row">
                                                                        ...
                                                                        <div class="col-md-6">
                                                                            <div class="form-group">
                                                                                ...
                                                                                <input type="text" name="sqrft"
                                                                                    id="sqrft" class="form-control"
                                                                                    value="{{ $productdata->SqrFt }}">
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="form-group">
    ...
    <script>
        ...
        $("#quantity, #sqrft").on("input", function() {
            // This function is triggered whenever the input value of #quantity or #sqrft changes.

            // Retrieve the values of #quantity and #sqrft and convert them to floating-point numbers.
            var quantity = parseFloat($("#quantity").val());
            var sqrft = parseFloat($("#sqrft").val());

            // Check if both quantity and sqrft are valid numbers.
            if (!isNaN(quantity) && !isNaN(sqrft)) {
                // If both are valid, calculate the total by multiplying quantity and sqrft.
                var total = quantity * sqrft;

                // Set the calculated total as the value of the element with the ID #total.
                $("#total").val(total);
            } else {
                // If either quantity or sqrft is not a valid number, set the value of #total to an empty string.
                $("#total").val("");
            }
        });
    </script>
